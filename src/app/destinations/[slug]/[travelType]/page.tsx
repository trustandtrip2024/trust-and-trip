...
export default async function DestinationTravelTypePage({ params }: Props) {
  ...
  return (
    <>
      ...
      {/* Hero */}
      <section className="relative h-[60vh] min-h-[420px] w-full overflow-hidden bg-tat-charcoal">
        <Image src={dest.heroImage} alt={`${dest.name} for ${cfg.label}`} fill priority className="object-cover" sizes="100vw" />
        <div className="absolute inset-0 bg-gradient-to-t from-tat-charcoal/95 via-tat-charcoal/50 to-tat-charcoal/40" />

        <div className="absolute top-24 inset-x-0 z-10 container-custom flex items-center gap-2 text-xs text-tat-paper/60 flex-wrap">
          ...
          <ChevronRight className="h-3 w-3" />
          <Link href={`/destinations/${dest.slug}`} className="hover:text-tat-gold">{dest.name}</Link>
          <ChevronRight className="h-3 w-3" />
          <span className="text-tat-gold">{cfg.label}</span>
        </div>

        <div className="relative h-full flex items-end pb-12 md:pb-16 container-custom text-tat-paper z-10">
          <div className="max-w-3xl">
            <span className="eyebrow text-tat-gold mb-3 block">{dest.name} · {dest.country}</span>
            <h1 className="font-display text-4xl md:text-6xl font-medium leading-[0.95] text-balance">
              {cfg.hero}
            </h1>
            <p className="mt-3 text-lg text-tat-paper/80 max-w-xl leading-relaxed">{cfg.intro}</p>
      ...
      <section className="py-14 container-custom">
        ...
        <div className="grid md:grid-cols-3 gap-5 max-w-4xl mx-auto">
          {cfg.whyCopy.map((p, i) => (
            <div key={i} className="bg-tat-paper rounded-2xl p-6 border border-tat-charcoal/8 shadow-soft">
              <div className="h-9 w-9 rounded-xl bg-tat-gold/15 text-tat-gold flex items-center justify-center font-display text-sm font-medium mb-4">
                {i + 1}
              </div>
              <p className="text-sm text-tat-charcoal/80 leading-relaxed">{p}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Packages */}
      <section className="bg-tat-cream py-14">
        <div className="container-custom">
          <div className="flex items-end justify-between flex-wrap gap-4 mb-8">
            <div>
              <span className="eyebrow text-tat-gold">Hand-picked</span>
              <h2 className="heading-section mt-1">{cfg.label} packages in {dest.name}</h2>
            </div>
            <Link href={`/destinations/${dest.slug}`} className="text-sm text-tat-charcoal/70 hover:text-tat-gold inline-flex items-center gap-1">
          ...
          {packages.length === 0 ? (
            <div className="bg-white rounded-2xl border border-tat-charcoal/8 p-10 text-center">
              <p className="text-tat-charcoal/60 mb-4">
      ...
      {/* CTA */}
      <section className="py-14 container-custom">
        <div className="bg-tat-charcoal rounded-3xl p-10 md:p-14 text-tat-paper text-center max-w-3xl mx-auto">
          <span className="eyebrow text-tat-gold">Customise it</span>
          <h2 className="heading-section text-tat-paper mt-2">
            None of these match your dates?
          </h2>
          <p className="mt-3 text-tat-paper/70 max-w-xl mx-auto leading-relaxed">
...
function Pill({ icon: Icon, label }: { icon: typeof IndianRupee; label: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 bg-tat-paper/12 border border-tat-paper/20 backdrop-blur-sm text-tat-paper rounded-full px-3.5 py-1.5 text-xs font-medium">
