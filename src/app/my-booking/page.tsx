  ...
  return (
    <div className={`rounded-2xl border-2 ${s.border} ${s.bg} p-6 space-y-5`}>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs text-ink/50 mb-1">Booking ID</p>
          <p className="font-mono text-sm text-ink/70">{b.id.slice(0, 8).toUpperCase()}…</p>
        </div>
      ...
      <div className="border-t border-ink/10 pt-4 space-y-3">
        <div className="flex items-start gap-2">
          <Package className="h-4 w-4 text-gold mt-0.5 shrink-0" />
          <div>
            <p className="text-xs text-ink/50">Package</p>
            <p className="font-medium text-ink">{b.package_title}</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {b.travel_date && (
            <div className="flex items-start gap-2">
              <CalendarDays className="h-4 w-4 text-gold mt-0.5 shrink-0" />
              <div>
                <p className="text-xs text-ink/50">Travel Date</p>
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs text-ink/50">Travellers</p>
      ...
      <div className="border-t border-ink/10 pt-4">
        <p className="text-xs text-ink/50 mb-3 uppercase tracking-wider">Payment Summary</p>
      ...
      <div className="container-custom max-w-lg">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="mt-3 font-display text-display-md font-medium leading-tight">
            Track your <span className="italic text-gold font-light">booking</span>
          </h1>
        ...
        {/* Search form */}
        <form onSubmit={submit} className="bg-white rounded-2xl border border-ink/10 p-6 shadow-sm mb-6">
          {/* Toggle */}
          <div className="flex gap-2 mb-5">
            {(["id", "phone"] as const).map((m) => (
              <button
                ...
                className={`flex-1 py-2 rounded-xl text-sm font-medium transition-all ${mode === m ? "bg-ink text-cream" : "bg-gray-100 text-ink/60 hover:bg-gray-200"}`}
              >
          ...
          <div className="relative">
            <input
              ...
              className="w-full border border-ink/15 rounded-xl px-4 py-3 pr-12 text-sm focus:outline-none focus:border-gold transition-colors"
              required
            />
            <Search className="absolute right-4 top-1/2 -translate-y-1/2 h-4 w-4 text-ink/30" />
          </div>
