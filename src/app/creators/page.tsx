...
export default function CreatorsPage() {
  return (
    <>
      {/* Hero */}
      <section className="relative pt-28 md:pt-36 pb-16 md:pb-24 bg-gradient-to-br from-ink via-ink to-ink/95 text-cream overflow-hidden">
        <div className="absolute -top-24 -right-24 h-96 w-96 rounded-full bg-gold/15 blur-3xl pointer-events-none" />
        <div className="absolute -bottom-32 -left-24 h-96 w-96 rounded-full bg-rose-500/10 blur-3xl pointer-events-none" />

        <div className="container-custom relative">
          <p className="text-[10px] uppercase tracking-[0.28em] text-gold/80 mb-4 flex items-center gap-2">
          ...
          <h1 className="font-display font-medium leading-[0.96] text-balance max-w-4xl"
            ...
            <br />
            into{" "}
            <span className="italic text-gold font-light">earnings.</span>
          </h1>
          ...
          <div className="mt-10 flex flex-wrap items-center gap-4">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 text-sm font-medium text-cream/85 hover:text-gold transition-colors border border-cream/20 hover:border-gold/40 px-5 py-2.5 rounded-full"
            >
              Talk to us first
            </a>
          </div>
          ...
          {/* Trust pills */}
          <div className="mt-12 flex flex-wrap items-center gap-x-8 gap-y-3 text-xs text-cream/55">
        ...
        <div className="container-custom">
          <div className="max-w-2xl mb-12">
            ...
            <h2 className="heading-section mt-2 text-balance">
              A real partnership,
              <span className="italic text-gold font-light"> not just a link.</span>
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
            {PERKS.map(({ icon: Icon, title, body }) => (
              <div key={title} className="bg-white rounded-2xl border border-ink/8 p-6 md:p-7 hover:shadow-soft transition-shadow">
                <div className="h-12 w-12 rounded-2xl bg-gold/15 text-gold flex items-center justify-center mb-4">
      ...
      {/* How it works */}
      <section className="py-20 md:py-28 bg-sand/25">
        <div className="container-custom">
          <div className="max-w-2xl mb-12 md:mb-16">
            <p className="eyebrow">How it works</p>
            <h2 className="heading-section mt-2 text-balance">
              Four steps. One signup.
              <span className="italic text-gold font-light"> Zero hard sell.</span>
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            {STEPS.map(({ num, title, body }) => (
              <div key={num} className="bg-white rounded-2xl border border-ink/8 p-6">
                <p className="font-display text-2xl text-gold/60 font-light">{num}</p>
                <h3 className="font-display text-lg font-medium text-ink mt-2 mb-2">{title}</h3>
        ...
        <div className="container-custom max-w-3xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={s.trip} className="bg-gold/8 border border-gold/20 rounded-2xl p-5">
                <p className="text-[11px] uppercase tracking-widest text-ink/50">{s.trip}</p>
                <p className="font-display text-base font-medium text-ink mt-1">{s.price}</p>
                <p className="font-display text-2xl text-gold mt-3">{s.earn}</p>
                <p className="text-[11px] text-ink/45 mt-0.5">your share (5%)</p>
              </div>
            ))}
          </div>
      ...
      <section className="py-20 md:py-28 bg-sand/25">
        <div className="container-custom max-w-3xl">
          ...
          <div className="space-y-3">
            {FAQ.map(({ q, a }) => (
              <details key={q} className="bg-white rounded-2xl border border-ink/8 p-5 group">
                <summary className="font-medium text-ink cursor-pointer flex items-center justify-between">
                  {q}
                  <ArrowRight className="h-4 w-4 text-ink/40 transition-transform group-open:rotate-90" />
                </summary>
                <p className="mt-3 text-sm text-ink/65 leading-relaxed">{a}</p>
              </details>
            ))}
          </div>
        </div>
