    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-tat-charcoal/8 p-10 text-center">
          <Heart className="h-10 w-10 text-tat-charcoal/20 mx-auto mb-3" />
          <p className="font-medium text-tat-charcoal">No saved trips yet</p>
          <p className="text-sm text-tat-charcoal/45 mt-1 mb-5">Tap the heart icon on any package to save it here.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {trips.map((trip) => (
            <div key={trip.id} className="bg-white rounded-2xl border border-tat-charcoal/8 overflow-hidden group hover:shadow-soft transition-all">
              ...
              <div className="p-4">
                <p className="text-[10px] uppercase tracking-widest text-tat-charcoal/40 mb-1">{trip.destination_name}</p>
                <h3 className="font-display text-base font-medium text-tat-charcoal leading-snug mb-2">{trip.package_title}</h3>
                <div className="flex items-center gap-3 text-xs text-tat-charcoal/45 mb-3">
                  <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{trip.duration}</span>
                </div>
                <div className="flex items-center justify-between border-t border-tat-charcoal/5 pt-3">
                  <p className="font-display text-lg text-tat-charcoal">
                    ₹{trip.package_price?.toLocaleString("en-IN")}
                    <span className="text-xs text-tat-charcoal/40 font-sans ml-1">/ person</span>
                  </p>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => moveToCart(trip)}
                      disabled={adding === trip.id}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-tat-gold/10 hover:bg-tat-gold text-tat-charcoal text-xs font-medium transition-all border border-tat-gold/20 hover:border-tat-gold"
                    >
                      {adding === trip.id ? (
                    ...
                    <Link
                      ...
                      className="h-8 w-8 rounded-xl bg-tat-charcoal flex items-center justify-center hover:bg-tat-gold transition-colors"
                    >
                      <ArrowRight className="h-3.5 w-3.5 text-tat-paper hover:text-tat-charcoal transition-colors" />
                    </Link>
                  </div>
                </div>
              </div>
