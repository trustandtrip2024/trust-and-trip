    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-tat-charcoal/8 p-10 text-center">
          <PenLine className="h-10 w-10 text-tat-charcoal/20 mx-auto mb-3" />
          <p className="font-medium text-tat-charcoal">No confirmed bookings yet</p>
          <p className="text-sm text-tat-charcoal/45 mt-1 mb-5">Reviews are available after your booking is confirmed.</p>
        ...
        <div className="space-y-3">
          {bookings.map((booking) => {
            ...
            return (
              <div key={booking.id} className="bg-white rounded-2xl border border-tat-charcoal/8 overflow-hidden">
                <div className="p-5 flex items-center justify-between gap-4">
                  ...
                  {!done && (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-tat-charcoal/15 text-sm font-medium text-tat-charcoal/70 hover:bg-tat-charcoal hover:text-tat-paper hover:border-tat-charcoal transition-all shrink-0"
                    >
                ...
                {open && !done && (
                  <form onSubmit={(e) => handleSubmit(e, booking)} className="px-5 pb-5 border-t border-tat-charcoal/5 pt-4 space-y-4">
                    {/* Star rating */}
                    <div>
                      <label className="block text-xs font-medium text-tat-charcoal/60 mb-2">Rating *</label>
                      <div className="flex items-center gap-1">
                        {[1,2,3,4,5].map((n) => (
                          <button key={n} type="button" onClick={() => setForm((f) => ({ ...f, rating: n }))}>
                            <Star className={`h-7 w-7 transition-colors ${n <= form.rating ? "fill-tat-gold text-tat-gold" : "text-tat-charcoal/20"}`} />
                          </button>
                        ))}
                        <span className="ml-2 text-sm font-medium text-tat-charcoal">{form.rating}/5</span>
                      </div>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="sm:col-span-2">
                        <label className="block text-xs font-medium text-tat-charcoal/60 mb-1.5">Review Title</label>
                        <input type="text" value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))}
                          placeholder="Summarise your experience"
                          className="w-full px-3.5 py-2.5 rounded-xl border border-tat-charcoal/15 bg-tat-paper text-sm text-tat-charcoal placeholder-tat-charcoal/35 focus:outline-none focus:ring-2 focus:ring-tat-gold/40 focus:border-tat-gold transition" />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-tat-charcoal/60 mb-1.5">Your City</label>
                        <input type="text" value={form.location} onChange={(e) => setForm((f) => ({ ...f, location: e.target.value }))}
                          placeholder="e.g. Delhi"
                          className="w-full px-3.5 py-2.5 rounded-xl border border-tat-charcoal/15 bg-tat-paper text-sm text-tat-charcoal placeholder-tat-charcoal/35 focus:outline-none focus:ring-2 focus:ring-tat-gold/40 focus:border-tat-gold transition" />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-tat-charcoal/60 mb-1.5">Travel Type</label>
                        <select value={form.travel_type} onChange={(e) => setForm((f) => ({ ...f, travel_type: e.target.value }))}
                          className="w-full px-3.5 py-2.5 rounded-xl border border-tat-charcoal/15 bg-tat-paper text-sm text-tat-charcoal focus:outline-none focus:ring-2 focus:ring-tat-gold/40 focus:border-tat-gold transition">
                          <option value="">Select…</option>
                          {["Couple","Family","Solo","Group","Friends"].map((t) => <option key={t} value={t}>{t}</option>)}
                        </select>
                      </div>
                      <div className="sm:col-span-2">
                        <label className="block text-xs font-medium text-tat-charcoal/60 mb-1.5">Your Review *</label>
                        <textarea value={form.body} onChange={(e) => setForm((f) => ({ ...f, body: e.target.value }))}
                          rows={4} placeholder="Tell others about your experience — what you loved, tips, highlights…"
                          className="w-full px-3.5 py-2.5 rounded-xl border border-tat-charcoal/15 bg-tat-paper text-sm text-tat-charcoal placeholder-tat-charcoal/35 focus:outline-none focus:ring-2 focus:ring-tat-gold/40 focus:border-tat-gold transition resize-none" />
                        <p className="text-[11px] text-tat-charcoal/35 mt-1">{form.body.length}/min 20 characters</p>
                      </div>
                    </div>
