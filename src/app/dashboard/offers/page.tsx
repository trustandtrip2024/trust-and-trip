  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <h1 className="font-display text-2xl font-medium text-ink">Offers for You</h1>
        <p className="text-sm text-ink/50 mt-1">Curated deals — updated daily just for members.</p>
      </div>
      ...
      {offers.length === 0 ? (
        <div className="bg-white rounded-2xl border border-ink/8 p-10 text-center">
          <Tag className="h-10 w-10 text-ink/20 mx-auto mb-3" />
          <p className="font-medium text-ink">No offers available right now</p>
          <p className="text-sm text-ink/45 mt-1">Check back soon for exclusive deals.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {offers.map((offer) => (
            <Link
              ...
              className="group bg-white rounded-2xl border border-ink/8 hover:border-ink/15 hover:shadow-soft overflow-hidden transition-all"
            >
              <div className="relative aspect-[16/9] overflow-hidden">
                ...
                <div className="absolute top-3 left-3 flex items-center gap-1.5">
                  <span className="inline-flex items-center gap-1 bg-gold text-ink text-[10px] font-bold uppercase px-2.5 py-1 rounded-full">
                    {offer.discount}% OFF
                  </span>
                  {offer.hot && (
                ...
                <div className="absolute bottom-3 left-3">
                  <span className="text-[10px] text-cream/80 bg-ink/50 backdrop-blur-sm px-2.5 py-1 rounded-full">{offer.tag}</span>
                </div>
              </div>

              <div className="p-4">
                <h3 className="font-display text-base font-medium text-ink group-hover:text-gold transition-colors line-clamp-2 mb-1.5">{offer.title}</h3>
                <div className="flex items-center gap-3 text-[11px] text-ink/45 mb-3">
                ...
                <div className="flex items-end justify-between">
                  <div>
                    <p className="text-[10px] text-ink/35 line-through mb-0.5">₹{offer.originalPrice.toLocaleString("en-IN")}</p>
                    <p className="font-display text-lg text-ink">₹{offer.price.toLocaleString("en-IN")}
                      <span className="text-xs text-ink/45 font-sans ml-1">/ person</span>
                    </p>
                  </div>
                  <div className="h-8 w-8 rounded-xl bg-gold/10 group-hover:bg-gold flex items-center justify-center transition-all">
