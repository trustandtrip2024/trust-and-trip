...
export default function BookingDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div>
      {/* Back */}
      <Link href="/dashboard/bookings" className="inline-flex items-center gap-1.5 text-sm text-tat-charcoal/55 hover:text-tat-charcoal mb-6 group">
      ...
      {booking.status !== "cancelled" && booking.status !== "refunded" && (
        <div className="bg-white rounded-2xl border border-tat-charcoal/8 p-5 md:p-6 mb-5">
          <p className="text-xs font-semibold text-tat-charcoal/60 uppercase tracking-wider mb-5">Progress</p>
                ...
                <div key={step.key} className="flex-1 flex flex-col items-center relative">
                  {i > 0 && (
                    <span
                      className={`absolute top-4 right-1/2 w-full h-0.5 ${
                        i <= currentStepIndex ? "bg-tat-gold" : "bg-tat-charcoal/10"
                      }`}
                      aria-hidden="true"
                    />
                  )}
                  <div
                    className={`relative z-10 h-8 w-8 rounded-full flex items-center justify-center transition-all ${
                        ...
                        : "bg-tat-charcoal/5 text-tat-charcoal/35"
                    } ${active ? "ring-4 ring-tat-gold/20" : ""}`}
                  >
                    <Icon className="h-4 w-4" />
                  </div>
                  <p className={`mt-2 text-[10px] md:text-xs font-medium text-center leading-tight ${done ? "text-tat-charcoal" : "text-tat-charcoal/40"}`}>
                    {step.label}
                  </p>
                </div>
              );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {/* Trip details */}
        <div className="md:col-span-2 bg-white rounded-2xl border border-tat-charcoal/8 p-5 md:p-6">
          <p className="text-xs font-semibold text-tat-charcoal/60 uppercase tracking-wider mb-4">Trip details</p>
          ...
          <div className="space-y-4">
            ...
            <Row icon={MapPin} label="Package">
              <Link href={`/packages/${booking.package_slug}`} className="text-tat-charcoal hover:text-tat-gold underline decoration-ink/20 hover:decoration-gold underline-offset-2">
                View package →
              </Link>
            </Row>
            {booking.special_requests && (
              <Row icon={AlertCircle} label="Special requests" align="start">
                <p className="text-sm text-tat-charcoal/70 leading-relaxed">{booking.special_requests}</p>
              </Row>
            )}
          </div>

          <div className="border-t border-tat-charcoal/8 mt-5 pt-5">
            <p className="text-xs font-semibold text-tat-charcoal/60 uppercase tracking-wider mb-4">Guest contact</p>
            <div className="space-y-3">
              <Row icon={Users} label="Name">{booking.customer_name}</Row>
              <Row icon={Mail} label="Email">
                <a href={`mailto:${booking.customer_email}`} className="text-tat-charcoal hover:text-tat-gold">{booking.customer_email}</a>
              </Row>
              <Row icon={Phone} label="Phone">
                <a href={`tel:${booking.customer_phone}`} className="text-tat-charcoal hover:text-tat-gold">{booking.customer_phone}</a>
              </Row>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-2xl border border-tat-charcoal/8 p-5 md:p-6">
            <p className="text-xs font-semibold text-tat-charcoal/60 uppercase tracking-wider mb-4">Payment</p>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-tat-charcoal/60">Package total</span>
                <span className="text-sm font-medium text-tat-charcoal">₹{(booking.package_price * booking.num_travellers).toLocaleString("en-IN")}</span>
              </div>
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between border-t border-tat-charcoal/8 pt-3">
                ...
                <span className="font-display text-lg font-medium text-tat-charcoal">₹{remainingAmount.toLocaleString("en-IN")}</span>
              </div>
              {booking.razorpay_payment_id && (
                <p className="text-[10px] text-tat-charcoal/35 pt-2 font-mono break-all">
          ...
          <div className="bg-white rounded-2xl border border-tat-charcoal/8 p-5 md:p-6">
            <p className="text-xs font-semibold text-tat-charcoal/60 uppercase tracking-wider mb-4">Actions</p>
            <div className="space-y-2">
              {(booking.status === "paid" || booking.status === "verified") && (
                <Link
                  href={`/dashboard/bookings/${booking.id}/receipt`}
                  target="_blank"
                  className="flex items-center justify-center gap-2 w-full bg-tat-charcoal hover:bg-tat-gold text-tat-paper hover:text-tat-charcoal py-2.5 rounded-xl text-sm font-semibold transition-all"
                >
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 w-full border border-whatsapp/30 bg-whatsapp/5 hover:bg-whatsapp/15 text-whatsapp-deep py-2.5 rounded-xl text-sm font-semibold transition-all"
              >
              ...
              {booking.status === "created" && (
                <Link
                  ...
                  className="flex items-center justify-center gap-2 w-full bg-tat-gold hover:bg-tat-gold/90 text-tat-charcoal py-2.5 rounded-xl text-sm font-semibold transition-all"
                >
          ...
          <div className="text-[11px] text-tat-charcoal/35 space-y-1 px-1">
...
function Row({
  ...
  return (
    <div className={`flex gap-3 ${align === "start" ? "items-start" : "items-center"}`}>
      <div className="h-8 w-8 rounded-lg bg-tat-charcoal/5 flex items-center justify-center shrink-0 mt-0.5">
        <Icon className="h-3.5 w-3.5 text-tat-charcoal/55" />
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-[10px] uppercase tracking-wider text-tat-charcoal/40 mb-0.5">{label}</p>
        <div className="text-sm text-tat-charcoal">{children}</div>
      </div>
    </div>
  );
}
