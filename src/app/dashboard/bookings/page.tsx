  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <h1 className="font-display text-2xl font-medium text-tat-charcoal">My Bookings</h1>
        <p className="text-sm text-tat-charcoal/50 mt-1">Your booking history and payment status.</p>
      </div>
          ...
          {bookings.map((b) => {
            ...
            return (
              <Link
                ...
                className="group block bg-white rounded-2xl border border-tat-charcoal/8 hover:border-tat-charcoal/15 hover:shadow-soft p-5 md:p-6 transition-all"
              >
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="min-w-0 flex-1">
                    ...
                    <h3 className="font-display text-base font-medium text-tat-charcoal leading-snug mt-1">{b.package_title}</h3>
                    <div className="flex items-center gap-4 mt-2 text-xs text-tat-charcoal/45 flex-wrap">
                      {b.travel_date && (
                    ...
                    {b.special_requests && (
                      <p className="text-xs text-tat-charcoal/40 mt-2 line-clamp-1">Notes: {b.special_requests}</p>
                    )}
                  </div>

                  <div className="text-right shrink-0">
                    <p className="text-xs text-tat-charcoal/40 mb-0.5">Deposit paid</p>
                    <p className="font-display text-lg font-medium text-tat-charcoal">₹{b.deposit_amount.toLocaleString("en-IN")}</p>
                    <p className="text-[11px] text-tat-charcoal/35 mt-0.5">of ₹{b.package_price.toLocaleString("en-IN")} total</p>
                    <span className="inline-flex items-center gap-1 text-[11px] text-tat-charcoal/45 mt-2 group-hover:text-tat-gold transition-colors">
