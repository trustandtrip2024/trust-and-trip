...
function Tab({
  ...
  return (
    ...
    >
      <div
        className={clsx(
          "flex items-center justify-center rounded-xl w-10 h-7 transition-all duration-200",
          active && "bg-gold/12"
        )}
      >
...
export default function MobileBottomNav() {
  ...
  return (
    ...
    >
      {/* Solid bar — fully opaque for readability */}
      <div className="bg-white border-t border-ink/8 shadow-[0_-4px_24px_rgba(11,28,44,0.10)]">
        <div className="flex items-end justify-around h-16 px-2 max-w-lg mx-auto">
          {leftTabs.map((t) => (
            <Tab key={t.href} {...t} active={isActive(t.href)} />
          ))}

          {/* Centre elevated WhatsApp FAB */}
          <a
            href={WHATSAPP}
            target="_blank"
          ...
          >
            ...
            <span className="text-[9px] uppercase tracking-[0.12em] font-medium text-ink/40 leading-none mt-0.5">
              Chat
            </span>
          </a>
