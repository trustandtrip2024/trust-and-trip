...
export default function ReviewsList({ packageSlug, initialCount = 0, initialRating }: Props) {
  ...
  if (reviews.length === 0) {
    return (
      <div className="bg-cream rounded-2xl p-8 border border-ink/5 text-center">
        <p className="text-ink/40 text-sm">No reviews yet — be the first to share your experience!</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {/* Summary bar */}
      {avg && (
        <div className="flex items-center gap-5 bg-cream rounded-2xl p-5 border border-ink/5">
          <div className="text-center">
            <p className="font-display text-5xl font-medium text-gold">{avg}</p>
            <div className="flex gap-0.5 mt-1 justify-center">
              {[1,2,3,4,5].map((s) => (
                <Star key={s} className={`h-3.5 w-3.5 ${s <= Math.round(Number(avg)) ? "fill-gold text-gold" : "text-ink/20"}`} />
              ))}
            </div>
            <p className="text-[11px] text-ink/40 mt-1">{count} review{count !== 1 ? "s" : ""}</p>
          </div>
            ...
            {[5,4,3,2,1].map((s) => {
              ...
              return (
                <div key={s} className="flex items-center gap-2">
                  <span className="text-[11px] text-ink/40 w-3">{s}</span>
                  ...
                  <span className="text-[11px] text-ink/40 w-6 text-right">{n}</span>
                </div>
              );
            })}
          </div>
      ...
      {/* Review cards */}
      {reviews.map((r) => (
        <div key={r.id} className="bg-white rounded-2xl p-5 border border-ink/6">
          {/* Header */}
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium text-sm text-ink">{r.reviewer_name}</p>
            ...
            <div className="flex gap-0.5 shrink-0">
              {[1,2,3,4,5].map((s) => (
                <Star key={s} className={`h-3.5 w-3.5 ${s <= r.rating ? "fill-gold text-gold" : "text-ink/15"}`} />
              ))}
            </div>
          </div>
            ...
            <button
              ...
              className={`flex items-center gap-1.5 text-[11px] px-3 py-1.5 rounded-full border transition-all ${
                helpedIds.has(r.id)
                  ...
                  : "border-ink/10 text-ink/40 hover:border-ink/25 hover:text-ink"
              }`}
            >
