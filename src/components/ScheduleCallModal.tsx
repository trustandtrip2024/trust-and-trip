...
import { X, Video, Phone, CheckCircle2, Loader2, ChevronLeft, Calendar } from "lucide-react";
...
export default function ScheduleCallModal({ packageTitle, packageSlug, destinationName, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div
        className="relative bg-cream rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="bg-cream border-b border-ink/8 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            {step === "form" && (
              <button
                onClick={() => setStep("choose")}
                className="h-7 w-7 rounded-full hover:bg-ink/8 flex items-center justify-center transition-colors mr-0.5"
              >
            ...
            <div>
              <p className="text-xs text-ink/45 uppercase tracking-widest">Schedule a Call</p>
              <p className="text-sm font-medium text-ink line-clamp-1">{packageTitle}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className="group flex flex-col items-center gap-3 p-5 rounded-2xl border-2 border-ink/10 hover:border-gold hover:bg-gold/5 transition-all duration-300"
                >
                  <div className="h-12 w-12 rounded-full bg-ink/5 group-hover:bg-gold/15 flex items-center justify-center transition-colors">
                    <Video className="h-5 w-5 text-ink/60 group-hover:text-ink transition-colors" />
                  </div>
                  <div className="text-center">
                    <p className="text-sm font-semibold text-ink">Video Call</p>
                    <p className="text-[11px] text-ink/45 mt-0.5">Google Meet / Zoom</p>
                  </div>
                </button>
                ...
                >
                  <div className="h-12 w-12 rounded-full bg-ink/5 group-hover:bg-gold/15 flex items-center justify-center transition-colors">
                    <Phone className="h-5 w-5 text-ink/60 group-hover:text-ink transition-colors" />
                  </div>
                  <div className="text-center">
                    <p className="text-sm font-semibold text-ink">Voice Call</p>
                    <p className="text-[11px] text-ink/45 mt-0.5">Phone / WhatsApp</p>
                  </div>
                </button>
              </div>
              <p className="text-[11px] text-ink/35 text-center mt-4">Free consultation · No obligation</p>
            </div>
          )}
          ...
          {step === "form" && (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-ink/60 mb-1.5">Preferred Time Slot</label>
          ...
          {step === "success" && (
            <div className="flex flex-col items-center gap-4 py-6 text-center">
              ...
              <div>
                <p className="font-display text-lg font-medium text-ink">Call scheduled!</p>
                <p className="text-sm text-ink/55 mt-1.5 leading-relaxed">
                  Our travel expert will call you at your preferred time.
                  {slot && <> Slot: <span className="font-medium text-ink">{slot}</span>.</>}
                </p>
              </div>
              <button
                onClick={onClose}
                className="mt-1 px-6 py-2.5 bg-ink text-cream rounded-full text-sm font-medium hover:bg-gold hover:text-ink transition-all"
              >
                Done
              </button>
            </div>
