    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-soft-lg max-w-md w-full max-h-[92vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-ink/8">
          <h2 className="font-display text-lg font-medium text-ink">Upload a document</h2>
        ...
        <form onSubmit={upload} className="p-5 space-y-4">
          <div>
            <label className="block text-xs font-medium text-ink/65 mb-1.5">Title *</label>
            <input
              type="text"
              value={title}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-ink/65 mb-1.5">Type</label>
              <select
                value={docType}
            ...
            <div>
              <label className="block text-xs font-medium text-ink/65 mb-1.5">Traveller (optional)</label>
              <select
                value={travellerId}
          ...
          <div>
            <label className="block text-xs font-medium text-ink/65 mb-1.5">File *</label>
            <label className="block border-2 border-dashed border-ink/15 hover:border-gold rounded-xl p-5 cursor-pointer transition-colors bg-cream/40">
              <input
                type="file"
                accept={ACCEPTED}
              ...
              {file ? (
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-ink/50">{(file.size / 1024).toFixed(0)} KB</p>
                  </div>
                </div>
              ) : (
                <div className="text-center">
                  <Upload className="h-6 w-6 text-ink/35 mx-auto mb-2" />
                  <p className="text-sm font-medium text-ink/70">Click to select file</p>
                  <p className="text-[11px] text-ink/45 mt-0.5">PDF, JPG, PNG, HEIC · max {MAX_SIZE_MB} MB</p>
                </div>
              )}
            </label>
          </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-ink hover:bg-gold text-cream hover:text-ink py-3 rounded-xl text-sm font-semibold transition-all disabled:opacity-60"
          >
