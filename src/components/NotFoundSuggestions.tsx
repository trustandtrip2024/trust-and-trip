...
import { Wand2, ArrowRight, Search, MapPin } from "lucide-react";
...
const FALLBACK = ["Bali", "Maldives", "Kerala", "Goa", "Dubai", "Thailand", "Manali", "Ladakh"];

// Client-side path-based suggestion. Lives outside not-found.tsx so the
// 404 page can stay statically renderable — reading `headers()` in
// not-found.tsx forces every route in the app into dynamic rendering,
// which kills ISR + edge cache for Meta-ad cold-clicks.
export default function NotFoundSuggestions() {
  ...
  const matches = path ? suggest(path) : [];
