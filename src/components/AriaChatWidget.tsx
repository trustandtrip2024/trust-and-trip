    ...
    <>
          ...
          >
            ...
            <div className="bg-ink px-4 py-3 flex items-center gap-3 shrink-0">
              <div className="relative shrink-0">
                <div className="h-10 w-10 rounded-full bg-[#f5c5a3] overflow-hidden flex items-end justify-center border-2 border-gold/40">
                  <AriaFace size={38} />
                </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-gold/20 text-gold uppercase tracking-wider font-medium">AI</span>
                </div>
                <p className="text-[11px] text-cream/50">Travel assistant · Online now</p>
              </div>
              <button onClick={() => setOpen(false)} className="text-cream/40 hover:text-cream transition-colors p-1">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"} items-end gap-2`}>
                  {m.role === "assistant" && (
                    <div className="h-7 w-7 rounded-full bg-[#f5c5a3] overflow-hidden flex items-end justify-center shrink-0 border border-gold/30">
                      <AriaFace size={26} />
                    </div>
                  )}
                  <div className={`max-w-[80%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-line ${
                    m.role === "user"
                      ...
                      : "bg-white text-ink border border-ink/6 rounded-bl-sm shadow-sm"
                  }`}>
                    {m.content}
                  </div>
                </div>
              ...
              {loading && (
                <div className="flex items-end gap-2">
                  ...
                  <div className="bg-white border border-ink/6 rounded-2xl rounded-bl-sm px-4 py-3 shadow-sm flex items-center gap-1">
                    {[0, 1, 2].map((i) => (
                      <motion.span key={i} className="h-1.5 w-1.5 rounded-full bg-ink/30 block"
                        animate={{ y: [0, -4, 0] }}
      ...
      <div className="fixed bottom-[4.5rem] md:bottom-6 left-4 md:left-6 z-50 flex items-center gap-2">
        ...
        <AnimatePresence>
          {showLabel && !open && (
            <motion.div
              ...
              className="bg-ink text-cream text-xs font-medium px-3 py-2 rounded-xl shadow-lg flex items-center gap-1.5 whitespace-nowrap pointer-events-none"
            >
        ...
        >
          ...
          {!open && (
            <>
              <span className="absolute inset-0 rounded-full bg-gold/20 animate-ping" style={{ animationDuration: "2.5s" }} />
              <span className="absolute inset-[-4px] rounded-full border border-gold/25" />
            </>
          )}
          ...
          <AnimatePresence mode="wait">
            {open ? (
              <motion.div key="close"
                ...
                className="h-[58px] w-[58px] rounded-full bg-ink flex items-center justify-center shadow-lg"
              >
              ...
              <motion.div key="face"
                ...
                className="h-[58px] w-[58px] rounded-full bg-[#f5c5a3] overflow-hidden flex items-end justify-center shadow-[0_4px_20px_rgba(245,158,11,0.35)] border-2 border-gold/60"
              >
                <AriaFace size={56} />
              </motion.div>
            )}
          ...
          {unread > 0 && !open && (
            <motion.span
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center shadow"
            >
              {unread}
            </motion.span>
          )}
