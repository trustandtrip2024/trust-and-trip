...
export default function TrustRibbon({
  ...
  const stats: Stat[] = [
    { icon: Star,             value: `${rating.toFixed(1)} ★`,        label: `${fmt(reviewCount)}+ Google reviews`, lottie: "/lottie/sparkle.json" },
    { icon: Users,            value: `${fmt(totalTravelers)}+`,       label: "happy travelers since 2019" },
    { icon: Clock,            value: "24 hours",                       label: "to your first itinerary",            lottie: "/lottie/pulse.json" },
  ...
  return (
    ...
    >
      <div className="container-custom py-5 md:py-7">
        ...
        >
          {stats.map(({ icon: Icon, value, label, lottie }) => (
            <li
              key={label}
            ...
            >
              <span className="shrink-0 h-9 w-9 rounded-full bg-tat-gold/15 text-tat-gold flex items-center justify-center">
                {lottie ? (
                  <LottieIcon src={lottie} size={22} />
                ) : (
                  <Icon className="h-4 w-4" aria-hidden />
                )}
              </span>
              <div className="min-w-0">
                <p className="text-[13px] md:text-[14px] font-semibold text-tat-charcoal dark:text-tat-paper leading-tight tabular-nums">
                  <CountUp value={value} />
                </p>
