...
const ITEMS = [
  ...
  { label: "Adventure",  href: "/experiences/adventure",  icon: Backpack, accent: "from-ember/15 to-ember/5         text-ember" },
  { label: "Luxury",     href: "/experiences/luxury",     icon: Crown,    accent: "from-gold/20 to-gold/5           text-gold-700" },
  ...
  { label: "Group",      href: "/experiences/group",      icon: Star,     accent: "from-crimson/15 to-crimson/4     text-crimson" },
    ...
    <section className="py-16 md:py-24" aria-labelledby="exp-grid-heading">
      <div className="container-custom">
        ...
        <div className="grid grid-cols-3 gap-3 md:gap-4">
          {ITEMS.map(({ label, href, icon: Icon, accent }) => (
            <Link
              key={href}
              href={href}
              className="group relative flex flex-col items-center justify-center gap-3 aspect-square md:aspect-[4/3] rounded-2xl md:rounded-3xl bg-cream border border-ink/8 hover:border-ember/40 hover:-translate-y-0.5 transition-all duration-300 overflow-hidden"
            >
              ...
              <p className="relative text-[11px] md:text-sm font-display font-semibold text-ink tracking-tight text-center leading-tight">
                {label}
              </p>
            </Link>
          ))}
        ...
        <div className="mt-6 flex md:hidden justify-center">
          <Link href="/experiences" className="text-sm font-medium text-ink/65 hover:text-ember transition-colors inline-flex items-center gap-1.5">
